...
//            std::cout<<"pico_message copy constructor being called.\n";
	...
	void convert_to_list_of_records() {
		...
		const char* temp_buffer_message = json_key_value_pair.c_str();
		...
		while (*temp_buffer_message != 0) {
			...
			currentRecord.parentMessageId = uniqueMessageId;

			for (int i = 0; i < pico_record::max_size - 12; i++) { // the last 12 character is for the offset of the next record that makes up the message
				currentRecord.sequenceNumber = numberOfRecords;
				if (*temp_buffer_message != 0) {
					currentRecord.data_[i] = *temp_buffer_message;
				} else {
					break;
				}
				++temp_buffer_message;
			...
			addTheEndingTagsToAllBuffers (currentRecord); //add append at the last characters of all, except the last one which completes the message
			std::cout
					<< "pico_message : currentRecord after append except the last one is "
