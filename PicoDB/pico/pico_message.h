        ...
        while (*temp_buffer_message != 0) {
         ...
         for (int i = 0; i < pico_buffer::max_size-6; i++) {
           ...
           ++temp_buffer_message;
            ...
            std::cout<<"pico_message : currentBuffer after append except the first one is "<<currentBuffer.toString();
            
            buffered_message.append(currentBuffer);
            
                 numberOfBuffer++;
             }
         removeTheEndingTags(buffered_message.getLastPointer());

         }
            void removeTheEndingTags(bufferType& currentBuffer)
            {
